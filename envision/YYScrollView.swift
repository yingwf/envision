...
class YYScrollView: UIView,UIScrollViewDelegate {
    ...
    private func initSubViews(){
        ...
        if (count == 0){
            ...
            firstImageView.imageFromUrl(self.sourceArr.last!)
        ...
        if (count > 0) {
            for index in 0 ... count - 1 {
                ...
                imageView.imageFromUrl(sourceArr[index])
        ...
        if (count > 0){
            let lastImageView = UIImageView(image: UIImage(named: "loading"))
            lastImageView.backgroundColor = UIColor.groupTableViewBackgroundColor()
            lastImageView.imageFromUrl(self.sourceArr.first!)
        ...
        if (count > 1) {
            ...
            self.timer = NSTimer.scheduledTimerWithTimeInterval(4, target: self, selector: "nextImage", userInfo: nil, repeats: true)
